...
GraphLink::GraphLink(int v)
{
    MaxVertex = v;
    NumVertex = NumEdge = 0;
    VertexTable = new Vertex[MaxVertex];
}
void GraphLink::InsertVertex(int v)
{
    if(NumVertex >= MaxVertex)
        return;
    VertexTable[NumVertex++].data = v;
}
int GraphLink::GetVertexI(int v)
{
...
int GraphLink::InsertEdge(int v1,int v2)
{
...
void GraphLink::Show()
{
    ...
    for(i = 0;i<NumVertex;i++)
    {
        ...
        while(p)
        {
            cout<<VertexTable[p->destvalue].data <<"->";
            p = p->link;
        }
        cout<<"nul"<<endl;
    }
}
void GraphLink::DeleteEdge(int v1,int v2)
{
    int p1 = GetVertexI(v1);
    int p2 = GetVertexI(v2);
    if(p1 == -1 || p2 == -1)
...
void GraphLink::DeleteVertex(int v)
{
    int p = GetVertexI(v);
    if(p == -1)
        return;
